...
@njit
def do_m2_intra(pos1, pos2, u_prev, u, x, x_post, v, y, lookup, lookup_next, lookup_prev):
    # update lookup table
    m2_lookup_intra_update(pos1, pos2, u, x, v, y, lookup, lookup_next)
    # update lookup_prev, lookup_next
    m2_lookup_precedence_update(lookup_prev, lookup_next, u_prev, u, x, x_post, v, y)


@njit
def m2_lookup_intra_update(pos1: int, pos2: int, u: int, x: int, v: int, y: int, lookup: np.ndarray,
    ...
    if pos1 + 1 < pos2:
        ...
        raise ValueError(f"Duplicated i, j: {u}, {v}")


@njit
def m2_lookup_precedence_update(lookup_prev, lookup_next, u_prev, u, x, x_post, v, y):
    """
